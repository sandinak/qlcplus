...
(function(){
  ...
  // Orientation and flame type
  algo.orientation = 0; // 0 bottom->top, 1 top->bottom
  algo.properties.push("name:orientation|type:list|display:Orientation|values:Bottom to Top,Top to Bottom|write:setOrientation|read:getOrientation");
  ...
  // Orientation accepts label strings from list
  algo.setOrientation = function(v){
    if (v === "Bottom to Top") { algo.orientation = 0; }
    else if (v === "Top to Bottom") { algo.orientation = 1; }
    else { var n = parseInt(v, 10); if (!isNaN(n) && (n === 0 || n === 1)) algo.orientation = n; }
  };
  algo.getOrientation = function(){ return (algo.orientation === 0) ? "Bottom to Top" : "Top to Bottom"; };
  ...
  algo.rgbMap = function(width, height, _rgb, _step){
    ...
    // Orientation direction sign for vertical travel: +1 = Bottom->Top (up), -1 = Top->Bottom (down)
    var dir = (algo.orientation === 0) ? 1 : -1;
    ...
    var feather = Math.max(1, Math.floor(effW * 0.1));
    ...
    var vfeather = Math.max(1, Math.floor(capH * 0.1));
	    ...
	    for (var wi2=0; wi2<wc; wi2++){
	      ...
	      w.xN += vx; if (w.xN < 0) w.xN = 0; if (w.xN > 1) w.xN = 1;
	      if (dir === 1){ if (w.yN > 1.2) w.yN -= 1.4; }
	      else { if (w.yN < -0.2) w.yN += 1.4; }
		      // Respawn plumes at base side with jitter and new randomness
		      if (dir === 1){
		        if (w.yN > 1.2){
		      ...
		      // Handle prior wrap by respawning with fresh randomness
		      if (dir === 1) {
		        if (w.yN < -0.2) {
		          w.yN = -0.05 + Math.random()*0.08;
		          w.xN = Math.random();
    ...
    for (var y=0;y<height;y++){
      ...
      for (var x=0;x<width;x++){
        ...
        if (!inside){
          ...
          var hFade = (hDist < feather) ? (hDist / feather) : 1.0;
